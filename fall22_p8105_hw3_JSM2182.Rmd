...
There are `r nrow(ny_noaa)` observations and `r ncol(ny_noaa)` variables. Specifically, there is weather data, such as the ID, the date, the preceipitation, snow, snow and wind, max temperature and minimum temperature.In terms of missing data, there are `r nrow(
  ny_noaa %>% 
  filter(
      is.na(prcp), 
      is.na(snow),
  ...
  summarize(
  across(prcp:tmin)) %>% 
  arrange(prcp, snow, snwd, tmax, tmin))` rows containing missing information. 
...
ny_noaa_tidy %>%
  ...
  scale_x_continuous(
    breaks = c(-10, 0, 15), #axes labels
    labels = c("-10C", "0", "15")  #labels are set as characters 
  ) 
