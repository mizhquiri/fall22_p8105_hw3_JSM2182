...
``` r
grouped by daysummarized by total count.fields(knitr::kable)
```
...
data, there are \_\_\_\_\_ rows containing missing information.
...
assessment of missing data per & code makes sense. answered.
...
interpretable structure? Any outliers? + labs( x = “Years”, y = “Maximum
Daily Temp (C)”, title = “Scatterplot of daily temp extremes in June and
July”, caption = “Data came from the Rnooa package”) +
...
If i don’t add id then it is more diffuse
...
tmin_noaa =
  ...
  ggplot(aes(x = year, y = tmin_mean, color = month)) +
  geom_smooth(alpha = 0.5)
```
...
```
    ...
    ## Warning: Removed 36342 rows containing non-finite values (stat_smooth).

    ## `geom_smooth()` using method = 'gam' and formula 'y ~ s(x, bs = "cs")'

    ## Warning: Removed 36339 rows containing non-finite values (stat_smooth).
...
than 0 and less than 100 separately by year ggplot(aes(x = year, y =
snow)) + geom_point(alpha = 0.5)
